...
export async function POST(req: Request) {
  ...
  if (!mimeType || !image) return new Response("Invalid image data", { status: 400 })

  if (!isSupportedImageType(mimeType)) {
    return new Response("Unsupported format. Only JPEG, PNG, GIF, and WEBP files are supported.", { status: 400 })
  }
