...
async function retryWithExponentialBackoff(fn: () => Promise<any>, maxRetries = 3): Promise<any> {
...
export async function POST(request: Request) {
  try {
    ...
    const recommendedOrientation = detectOptimalOrientation(input)

    try {
      const userPrompt = `Generate a Mermaid flowchart code for the following description: ${input}
...
Ensure all node connections are logical and flow naturally.`
      ...
      const response = await retryWithExponentialBackoff(async () => {
        return await callClaudeAPI(userPrompt, systemPrompt, 1000)
      })
      ...
      const simpleMermaidCode = generateSimpleFlowchart(input, recommendedOrientation)
      return NextResponse.json({
