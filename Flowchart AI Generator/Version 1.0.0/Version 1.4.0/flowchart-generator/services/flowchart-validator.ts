...
import { cleanMermaidCode, fixMermaidSyntaxIssues, ensureValidGraphDeclaration } from "@/utils/mermaid-helpers"
 ...
 */
export async function validateAndCorrectMermaidCode(mermaidCode: string): Promise<MermaidValidationResult> {
  try {
    ...
    // Check if the code starts with graph or flowchart
    if (!trimmedCode.match(/^(?:```(?:mermaid)?\s*)?(?:graph\s+(?:TB|TD|BT|RL|LR)|flowchart\s+(?:TB|TD|BT|RL|LR))/i)) {
      ...
      // If we can't extract a declaration, add a default one
      const correctedCode = ensureValidGraphDeclaration(cleanedCode)
      return {
        correctedCode,
        wasFixed: true,
        error: "Added missing graph declaration",
      }
    }
...
`
    ...
    const response = await callClaudeAPI(userPrompt, systemPrompt, { maxTokens: 1000 })
    const correctedCode = response.content[0].text.trim()

    // Check if Claude made any changes
    const wasFixed = correctedCode !== cleanedCode
    ...
    // Apply local fixes as a fallback
    const fixedCode = fixMermaidSyntaxIssues(cleanMermaidCode(mermaidCode))

    return {
      correctedCode: fixedCode,
      wasFixed: fixedCode !== mermaidCode,
